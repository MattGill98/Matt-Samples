...
import static uk.me.mattgill.samples.jacc.TestPolicyConfigurationStateMachine.State.DELETED;
import static uk.me.mattgill.samples.jacc.TestPolicyConfigurationStateMachine.State.INSERVICE;
import static uk.me.mattgill.samples.jacc.TestPolicyConfigurationStateMachine.State.OPEN;
...
public class TestPolicyConfigurationStateMachine implements PolicyConfiguration {
    ...
    //
    // commit() can only be called when the state is OPEN or INSERVICE and next
    // state is always INSERVICE
    ...
    /**
     * Transition back to open. This method is required because of the
     * {@link PolicyConfigurationFactory} contract, but is mysteriously missing from
     * the interface.
     */
    public void open() {
        state = OPEN;
