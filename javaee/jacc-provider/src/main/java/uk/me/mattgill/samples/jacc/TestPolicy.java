...
import static uk.me.mattgill.samples.jacc.TestPolicyConfigurationFactory.getCurrentPolicyConfiguration;
...
public class TestPolicy extends Policy {
    ...
    public boolean implies(ProtectionDomain domain, Permission permission) {
        ...
        if (!roleMapper.isAnyAuthenticatedUserRoleMapped() && !currentUserPrincipals.isEmpty()) {
            // The "any authenticated user" role is not mapped, so available to anyone and
            // the current
            // user is assumed to be authenticated (we assume that an unauthenticated user
            // doesn't have any principals
            // whatever they are)
            if (hasAccessViaRole(policyConfiguration.getPerRolePermissions(), "**", permission)) {
                // Access is granted purely based on the user being authenticated (the actual
                // roles, if any, the user has it not important)
                return true;
            }
        }
        ...
        if (hasAccessViaRoles(policyConfiguration.getPerRolePermissions(),
                roleMapper.getMappedRoles(currentUserPrincipals, subject), permission)) {
            // Access is granted via role. Note that if this returns false it doesn't mean
            // the permission is not
            // granted. A role can only grant, not take away permissions.
            return true;
        }

        // Access not granted via any of the JACC maintained Permissions. Check the
        // previous (default) policy.
        // Note: this is likely to be called in case it concerns a Java SE type
        // permissions.
        // TODO: Should we not distinguish between JACC and Java SE Permissions at the
        // start of this method? Seems
        // very unlikely that JACC would ever say anything about a Java SE Permission,
        // or that the Java SE
        // policy says anything about a JACC Permission. Why are these two systems even
    ...
    public PermissionCollection getPermissions(ProtectionDomain domain) {
        ...
        //
        // Note that the principles that are put into the ProtectionDomain object are
        // those from the current user.
        // (for a Server application, passing in a Subject would have been more logical,
        // but the Policy class was
        // made for Java SE with code-level security in mind). The Subject needs to be
        // passed anyway as some servers
    ...
    public PermissionCollection getPermissions(CodeSource codesource) {
        ...
        // Secondly get the static permissions. Note that there are only two sources
        // possible here, without
        // knowing the roles of the current user we can't check the per role
        // permissions.
        collectPermissions(policyConfiguration.getUncheckedPermissions(), permissions, excludedPermissions);
    ...
    private boolean hasAccessViaRoles(Map<String, Permissions> perRolePermissions, List<String> roles,
    ...
    /**
     * Copies permissions from a source into a target skipping any permission that's
     * excluded.
     *
